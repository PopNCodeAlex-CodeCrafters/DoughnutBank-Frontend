...
export default class DiffieHellman {
  ...
  private computeDerivedKey(otherPartyPublicKey: string): string {
    return this.diffieHellman.generateSharedSecret(otherPartyPublicKey);
  }

  private encryptUsingAes(message: string, aesKey: string): string {
  ...
  private decryptUsingAes(encryptedMessage: string, aesKey: string, iv: string): string {
